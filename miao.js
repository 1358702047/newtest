...
function timer()
{
	//获取商品的秒杀时间-根据属性获取-获取到字符串"{time:1414986097, item:'2839496677'}"
	//-单位秒
...
window.onload = function() {
	...
	Ajax.request({
			url:'http://localhost/miao/timer.php',
			params:{userid:uid,id:itemid},
			type:'json',
			callback: canBuy 
		});
}
...
function canBuy(obj) {
	if (obj.errno == 0  && obj.data.available!=1) {
		...
		document.getElementById("item-time").innerHTML = '已抢光';
		document.getElementById("item-time").style.backgroundColor = 'gray';
	}
}
